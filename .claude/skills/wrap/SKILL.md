...
- **Verde:** `add`, `commit` e **`push`, sem perguntar** — não peça permissão para o que já está decidido (DEC-033, que revogou o desvio da DEC-032: em nove aplicações a confirmação foi dada nove vezes, nunca impediu nada, e produzia pergunta em prosa). **O push se resolve ANTES do relatório:** o relatório é o ÚLTIMO passo e diz o que de fato aconteceu — empurrado, com o hash. Nunca deixe uma versão do relatório escrita antes do push valendo como registro.
- **Divergência entre previsto e medido NÃO é vermelho** e não bloqueia o push. Ela entra em **seção própria do relatório**, com o número previsto, o medido e a causa se você a souber — o medido sempre vence, e não se conserta o arquivo para «fechar» o número.
